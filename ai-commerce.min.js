...
`}function G(t){let r=document.createElement("style");r.id="aicommerce-widget-styles",r.textContent=t;let e=document.getElementById("aicommerce-widget-styles");return e&&e.remove(),document.head.appendChild(r),r}var _=B(()=>{});var Q={};X(Q,{AICommerceWidget:()=>exports.AICommerceWidget,createWidget:()=>z});function z(t){if(!t.apiKey)throw new Error("AICommerceWidget: apiKey is required");let r=new exports.AICommerce({apiKey:t.apiKey,storeId:t.storeId,baseUrl:t.baseUrl}),e={isOpen:false,isLoading:true,isRecording:false,messages:[],storeConfig:null},c=null,h=[],s=null,y=null,p;async function Z(){try{let o=t.baseUrl||K(),a=await fetch(`${o}/api/v1/store`,{headers:{"x-api-key":t.apiKey}});return a.ok?(await a.json()).store:null}catch(o){return console.error("Failed to fetch store config:",o),null}}function K(){if(typeof window<"u"){let o=document.querySelector("script[data-aicommerce-url]");if(o)return o.getAttribute("data-aicommerce-url")||""}return "https://api.aicommerce.dev"}async function ee(){e.storeConfig=await Z();let o=t.displayMode||"widget",a=o==="embedded";p={apiKey:t.apiKey,storeId:t.storeId,baseUrl:t.baseUrl||K(),displayMode:o,container:t.container,maxHeight:t.maxHeight||"600px",placeholder:t.placeholder||"Ask me anything about our products...",position:t.position||"bottom-right",theme:t.theme||"auto",primaryColor:t.primaryColor||e.storeConfig?.primaryColor||"#6366f1",welcomeMessage:t.welcomeMessage||e.storeConfig?.welcomeMessage||"Hi! How can I help you find the perfect product today?",botName:t.botName||e.storeConfig?.chatBotName||"Shopping Assistant",zIndex:t.zIndex||9999,buttonText:t.buttonText||"\u{1F4AC}",hideLauncher:t.hideLauncher||false,onOpen:t.onOpen,onClose:t.onClose,onProductClick:t.onProductClick,onMessage:t.onMessage};let d=j(p);if(y=G(d),a){let b=null;if(typeof t.container=="string"?b=document.querySelector(t.container):t.container instanceof HTMLElement&&(b=t.container),!b){console.error("[AI Commerce] Embedded mode requires a valid container element or selector");return}s=document.createElement("div"),s.id="aicommerce-widget",s.className=`aicommerce-widget aicommerce-embedded aicommerce-theme-${p.theme}`,s.style.setProperty("--aic-max-height",p.maxHeight),b.appendChild(s),e.isOpen=true;}else s=document.createElement("div"),s.id="aicommerce-widget",s.className=`aicommerce-widget aicommerce-${p.position} aicommerce-theme-${p.theme}`,document.body.appendChild(s);x(),a||e.messages.push({role:"assistant",content:p.welcomeMessage}),e.isLoading=false,x();}function x(){if(!s)return;let o=p.displayMode==="embedded",a=e.messages.some(l=>l.role==="user");o&&(s.classList.remove("aicommerce-no-messages","aicommerce-has-messages"),s.classList.add(a?"aicommerce-has-messages":"aicommerce-no-messages"));let d=p.placeholder||"Ask me anything about our products...",b=`
                                    ...
                                    ${l.products.map(i=>`
                                        <div class="aicommerce-product-card" data-product-id="${i.id}">
                                            ...
                                            <div class="aicommerce-product-info">
                                                ...
                                                <span class="aicommerce-product-price">${V(i.price,i.currency)}</span>
                                            </div>
                                        </div>
                                    `).join("")}
                                </div>
        ...
        `;s.innerHTML=u,re();let k=s.querySelector(".aicommerce-messages");k&&(k.scrollTop=k.scrollHeight);}function N(o,a,d){return `
            <div class="aicommerce-audio-player" data-message-index="${a}">
                ...
                <div class="aicommerce-audio-waveform">
                    ...
                    <div class="aicommerce-audio-time">
                        ...
                        <span>${J(o.audioDuration||0)}</span>
                    </div>
                </div>
                <audio src="${o.audioUrl}" preload="metadata"></audio>
            </div>
        `}function re(){if(!s)return;let o=s.querySelector(".aicommerce-launcher");o&&o.addEventListener("click",()=>M());let a=s.querySelector(".aicommerce-close");a&&a.addEventListener("click",()=>U());let d=s.querySelector(".aicommerce-input"),b=s.querySelector(".aicommerce-send");d&&(d.addEventListener("keydown",n=>{let m=n;if(m.key==="Enter"){let i=d instanceof HTMLTextAreaElement;if(i&&m.shiftKey)return;n.preventDefault();let f=d.value.trim();f&&(A(f),d.value="",i&&(d.style.height="auto"));}}),d instanceof HTMLTextAreaElement&&d.addEventListener("input",()=>{d.style.height="auto",d.style.height=Math.min(d.scrollHeight,150)+"px";})),b&&d&&b.addEventListener("click",()=>{let n=d.value.trim();n&&(A(n),d.value="",d instanceof HTMLTextAreaElement&&(d.style.height="auto"));});let u=s.querySelector(".aicommerce-mic");u&&u.addEventListener("click",()=>te()),s.querySelectorAll(".aicommerce-product-card").forEach(n=>{n.addEventListener("click",()=>{let m=n.getAttribute("data-product-id"),i=e.messages.flatMap(f=>f.products||[]).find(f=>f.id===m);i&&(p.onProductClick?p.onProductClick(i):i.url&&window.open(i.url,"_blank","noopener,noreferrer"));});}),s.querySelectorAll(".aicommerce-products").forEach(n=>{let m=false,i=0,f=0;n.addEventListener("mousedown",E=>{m=true,n.style.cursor="grabbing",i=E.pageX-n.offsetLeft,f=n.scrollLeft;}),n.addEventListener("mouseleave",()=>{m=false,n.style.cursor="grab";}),n.addEventListener("mouseup",()=>{m=false,n.style.cursor="grab";}),n.addEventListener("mousemove",E=>{if(!m)return;E.preventDefault();let v=(E.pageX-n.offsetLeft-i)*2;n.scrollLeft=f-v;});}),s.querySelectorAll(".aicommerce-audio-player").forEach(n=>{let m=n.querySelector("audio"),i=n.querySelector(".aicommerce-audio-btn"),f=n.querySelectorAll(".aicommerce-waveform-bar"),E=n.querySelector(".aicommerce-current-time");if(!m||!i)return;i.addEventListener("click",()=>{!m.paused?(m.pause(),i.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>'):(s?.querySelectorAll("audio").forEach(C=>{if(C!==m&&!C.paused){C.pause();let I=C.closest(".aicommerce-audio-player")?.querySelector(".aicommerce-audio-btn");I&&(I.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>');}}),m.play(),i.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2"><rect x="6" y="4" width="4" height="16"></rect><rect x="14" y="4" width="4" height="16"></rect></svg>');}),m.addEventListener("timeupdate",()=>{if(E&&(E.textContent=J(m.currentTime)),m.duration){let v=m.currentTime/m.duration*100;f.forEach((C,q)=>{q/f.length*100<=v?C.style.backgroundColor=n.closest(".aicommerce-user")?"rgba(255,255,255,1)":"var(--aic-primary)":C.style.backgroundColor=n.closest(".aicommerce-user")?"rgba(255,255,255,0.4)":"rgba(99,102,241,0.3)";});}}),m.addEventListener("ended",()=>{i.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>';});let S=n.querySelector(".aicommerce-waveform-bars");S&&S.addEventListener("click",v=>{let C=S.getBoundingClientRect(),I=(v.clientX-C.left)/C.width;m.duration&&(m.currentTime=I*m.duration);});});}async function te(){if(e.isRecording)c&&c.state!=="inactive"&&c.stop();else try{let o=await navigator.mediaDevices.getUserMedia({audio:!0});h=[],c=new MediaRecorder(o,{mimeType:MediaRecorder.isTypeSupported("audio/webm")?"audio/webm":"audio/mp4"}),c.ondataavailable=a=>{a.data.size>0&&h.push(a.data);},c.onstop=async()=>{if(o.getTracks().forEach(a=>a.stop()),h.length>0){let a=new Blob(h,{type:c?.mimeType||"audio/webm"});await oe(a);}e.isRecording=!1,x();},c.start(),e.isRecording=!0,x();}catch(o){console.error("Failed to start recording:",o),e.messages.push({role:"assistant",content:"Unable to access microphone. Please check your permissions."}),x();}}async function oe(o){let a=URL.createObjectURL(o),d=Array(40).fill(10),b=0;try{d=await ae(o);let u=new Audio(a);await new Promise(k=>{u.onloadedmetadata=()=>{b=u.duration,k();},u.onerror=()=>k();});}catch(u){console.error("Audio analysis failed",u);}e.messages.push({role:"user",content:"Voice message",audioUrl:a,audioDuration:b,waveformBars:d}),e.isLoading=true,x();try{let u=await r.chatWithAudio(o);return e.messages.push({role:"assistant",content:u.reply,products:u.products}),p.onMessage&&p.onMessage("Voice message",u),u}catch(u){throw e.messages.push({role:"assistant",content:"Sorry, I encountered an error processing your voice message. Please try again."}),u}finally{e.isLoading=false,x();}}function Y(o){return /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/.test(o)}function J(o){let a=Math.floor(o/60),d=Math.floor(o%60);return `${a}:${d.toString().padStart(2,"0")}`}async function ae(o){try{let a=new(window.AudioContext||window.webkitAudioContext),d=await o.arrayBuffer(),u=(await a.decodeAudioData(d)).getChannelData(0),k=40,l=Math.floor(u.length/k),$=[];for(let n=0;n<k;n++){let m=n*l,i=m+l,f=0;for(let v=m;v<i;v++)u[v]&&(f+=u[v]*u[v]);let E=Math.sqrt(f/l),S=Math.min(100,Math.max(10,E*400));$.push(S);}return $}catch(a){return console.error("Analysis error",a),Array.from({length:40},()=>20+Math.random()*60)}}async function A(o){e.messages.push({role:"user",content:o}),e.isLoading=true,x();try{let a=await r.chat(o);return e.messages.push({role:"assistant",content:a.reply,products:a.products}),p.onMessage&&p.onMessage(o,a),a}catch(a){throw e.messages.push({role:"assistant",content:"Sorry, I encountered an error. Please try again."}),a}finally{e.isLoading=false,x();}}function M(){e.isOpen=true,x(),p.onOpen?.(),setTimeout(()=>{s?.querySelector(".aicommerce-input")?.focus();},100);}function U(){e.isOpen=false,x(),p.onClose?.();}function ie(){e.isOpen?U():M();}function se(){s&&(s.remove(),s=null),y&&(y.remove(),y=null);}function ne(o){if(Object.assign(p,o),o.primaryColor){let a=j(p);y&&(y.textContent=a);}x();}function w(o){let a=document.createElement("div");return a.textContent=o,a.innerHTML}function V(o,a){let b={USD:"$",EUR:"\u20AC",GBP:"\xA3",MAD:"DH",SAR:"SAR",AED:"AED",JPY:"\xA5",CNY:"\xA5"}[a||"USD"]||a||"$";return `${o.toFixed(2)} ${b}`}return ee(),{open:M,close:U,toggle:ie,destroy:se,sendMessage:A,updateConfig:ne}}exports.AICommerceWidget=void 0;var F=B(()=>{R();_();exports.AICommerceWidget={init:z,VERSION:"1.0.0"};typeof window<"u"&&(window.AICommerceWidget=exports.AICommerceWidget);});R();F();var fe="1.0.0";typeof window<"u"&&(window.AICommerce=(R(),H(W)).AICommerce,window.AICommerceError=(R(),H(W)).AICommerceError,window.AICommerceWidget=(F(),H(Q)).AICommerceWidget);exports.VERSION=fe;exports.createWidget=z;return exports;})({});//# sourceMappingURL=index.min.js.map
