...
`}function _(o){let r=document.createElement("style");r.id="aicommerce-widget-styles",r.textContent=o;let e=document.getElementById("aicommerce-widget-styles");return e&&e.remove(),document.head.appendChild(r),r}var Q=H(()=>{});var Z={};G(Z,{AICommerceWidget:()=>exports.AICommerceWidget,createWidget:()=>D});function D(o){if(!o.apiKey)throw new Error("AICommerceWidget: apiKey is required");let r=new exports.AICommerce({apiKey:o.apiKey,storeId:o.storeId,baseUrl:o.baseUrl}),e={isOpen:false,isLoading:true,isRecording:false,messages:[],storeConfig:null},c=null,h=[],s=null,w=null,p;async function ee(){try{let a=o.baseUrl||N(),i=await fetch(`${a}/api/v1/store`,{headers:{"x-api-key":o.apiKey}});return i.ok?(await i.json()).store:null}catch(a){return console.error("Failed to fetch store config:",a),null}}function N(){if(typeof window<"u"){let a=document.querySelector("script[data-aicommerce-url]");if(a)return a.getAttribute("data-aicommerce-url")||""}return "https://api.aicommerce.dev"}async function re(){e.storeConfig=await ee();let a=o.displayMode||"widget",i=a==="embedded";p={apiKey:o.apiKey,storeId:o.storeId,baseUrl:o.baseUrl||N(),displayMode:a,container:o.container,maxHeight:o.maxHeight||"600px",placeholder:o.placeholder||"Ask me anything about our products...",position:o.position||"bottom-right",theme:o.theme||"auto",primaryColor:o.primaryColor||e.storeConfig?.primaryColor||"#6366f1",welcomeMessage:o.welcomeMessage||e.storeConfig?.welcomeMessage||"Hi! How can I help you find the perfect product today?",botName:o.botName||e.storeConfig?.chatBotName||"Shopping Assistant",zIndex:o.zIndex||9999,buttonText:o.buttonText||"\u{1F4AC}",hideLauncher:o.hideLauncher||false,onOpen:o.onOpen,onClose:o.onClose,onProductClick:o.onProductClick,onAddToCart:o.onAddToCart,onMessage:o.onMessage};let d=z(p);if(w=_(d),i){let b=null;if(typeof o.container=="string"?b=document.querySelector(o.container):o.container instanceof HTMLElement&&(b=o.container),!b){console.error("[AI Commerce] Embedded mode requires a valid container element or selector");return}s=document.createElement("div"),s.id="aicommerce-widget",s.className=`aicommerce-widget aicommerce-embedded aicommerce-theme-${p.theme}`,s.style.setProperty("--aic-max-height",p.maxHeight),b.appendChild(s),e.isOpen=true;}else s=document.createElement("div"),s.id="aicommerce-widget",s.className=`aicommerce-widget aicommerce-${p.position} aicommerce-theme-${p.theme}`,document.body.appendChild(s);k(),i||e.messages.push({role:"assistant",content:p.welcomeMessage}),e.isLoading=false,k();}async function te(a,i=1){if(!(await fetch("/cart/add.js",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:a,quantity:i})})).ok)throw new Error("Failed to add to cart");document.dispatchEvent(new CustomEvent("cart:refresh"));}function k(){if(!s)return;let a=p.displayMode==="embedded",i=e.messages.some(m=>m.role==="user");a&&(s.classList.remove("aicommerce-no-messages","aicommerce-has-messages"),s.classList.add(i?"aicommerce-has-messages":"aicommerce-no-messages"));let d=p.placeholder||"Ask me anything about our products...",b=`
        ...
        `}function oe(){if(!s)return;let a=s.querySelector(".aicommerce-launcher");a&&a.addEventListener("click",()=>q());let i=s.querySelector(".aicommerce-close");i&&i.addEventListener("click",()=>O());let d=s.querySelector(".aicommerce-input"),b=s.querySelector(".aicommerce-send");d&&(d.addEventListener("keydown",n=>{let t=n;if(t.key==="Enter"){let l=d instanceof HTMLTextAreaElement;if(l&&t.shiftKey)return;n.preventDefault();let f=d.value.trim();f&&(U(f),d.value="",l&&(d.style.height="auto"));}}),d instanceof HTMLTextAreaElement&&d.addEventListener("input",()=>{d.style.height="auto",d.style.height=Math.min(d.scrollHeight,150)+"px";})),b&&d&&b.addEventListener("click",()=>{let n=d.value.trim();n&&(U(n),d.value="",d instanceof HTMLTextAreaElement&&(d.style.height="auto"));});let u=s.querySelector(".aicommerce-mic");u&&u.addEventListener("click",()=>ae()),s.querySelectorAll(".aicommerce-product-card").forEach(n=>{n.addEventListener("click",t=>{if(t.target.closest(".aicommerce-add-to-cart"))return;let l=n.getAttribute("data-product-id"),f=e.messages.flatMap(y=>y.products||[]).find(y=>y.id===l);f&&(p.onProductClick?p.onProductClick(f):f.url&&window.open(f.url,"_blank","noopener,noreferrer"));});}),s.querySelectorAll(".aicommerce-add-to-cart").forEach(n=>{n.addEventListener("click",async t=>{t.stopPropagation();let l=n,f=l.closest(".aicommerce-product-card"),y=f?.getAttribute("data-product-id"),x=f?.getAttribute("data-variant-id"),C=e.messages.flatMap(P=>P.products||[]).find(P=>P.id===y);if(!C)return;let v=l.innerHTML;l.disabled=true,l.innerHTML=`
